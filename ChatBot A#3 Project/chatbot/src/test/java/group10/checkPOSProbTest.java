...
public class checkPOSProbTest {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"who is your wife", "what is deadpools imdb"}) //add a whole bunch of tests HERE*******************
    public void checkPOSProbIsTRUE(String input) throws IOException
    {
        ...
        // calling the postag method with the tokenized input to initialize the probability variable
        POSTagging.posTag(Tokenizer.tokens);
        // check if the probability method with the probs double[] array returns true, and check if it matches true to indicate test passed with correct input
...
//----------------------------------------------------------------------------------------------------------------------------
    ...
    public void checkPOSProbIsFALSE(String input) throws IOException
    {
       ...
       // calling the postag method with the tokenized input to initialize the probability variable
       POSTagging.posTag(Tokenizer.tokens);
       // check if the probability method with the probs double[] array returns false, and check if it matches false to indicate test passed with incorrect input
       assertEquals(ChatBot.checkPOSProb(POSTagging.probs), true);
    }
}
