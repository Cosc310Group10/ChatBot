...
//testMap
     ...
     static HashMap<String, Double> imdbMap = new HashMap<String, Double>();
    //Initializing the year map
    static HashMap<String, Integer> yearMap = new HashMap<String, Integer>();
    //Initializing the rating map
    static HashMap<String, String> ratingMap = new HashMap<String, String>();
    //Initializing the cast map
    static HashMap<String, ArrayList> castMap = new HashMap<String, ArrayList>();
    ...
    static HashMap<String, ArrayList> awardsMap = new HashMap<String, ArrayList>();
  ...
  public static <T> void fillInMap(HashMap<String,String> map,String value){
    
    for(int i=0; i<listOfMovies.size();i++){

      if(value == "imdb"){
        map.put(listOfMovies.get(i).getTitle()," has " + listOfMovies.get(i).getImdbStars()+" IMDb stars!");

      }else if(value =="year"){
        map.put(listOfMovies.get(i).getTitle(),listOfMovies.get(i).getYear());

      }else if(value =="rating"){
        map.put(listOfMovies.get(i).getTitle(),listOfMovies.get(i).getRating());

      }
      else if(value =="cast"){
        map.put(listOfMovies.get(i).getTitle(),listOfMovies.get(i).getCast());

      }
      else if(value =="director"){
        map.put(listOfMovies.get(i).getTitle(),listOfMovies.get(i).getDirector());

      }
      else if(value =="genre"){
        map.put(listOfMovies.get(i).getTitle(),listOfMovies.get(i).getGenre());

      }
      else if(value =="awards"){
        map.put(listOfMovies.get(i).getTitle(),listOfMovies.get(i).getAwards());

      }
      else if(value =="box office"){
        map.put(listOfMovies.get(i).getTitle(),listOfMovies.get(i).getBoxOffice());

      }
      else if(value =="location"){
        map.put(listOfMovies.get(i).getTitle(),listOfMovies.get(i).getLocation());

      }
      else if(value =="time"){
        map.put(listOfMovies.get(i).getTitle(),listOfMovies.get(i).getTimeToFilm());

      }
      else if(value =="duration"){
        map.put(listOfMovies.get(i).getTitle(),listOfMovies.get(i).getDuration());

      }
      else if(value =="budget"){
        map.put(listOfMovies.get(i).getTitle(),listOfMovies.get(i).getBudgetOfMovie());
  ...
  public static void analyzeInput(String userInput) {
   ...
   for(int i=0; i<listOfMovies.size();i++){
       if(userInput.contains(listOfMovies.get(i).getTitle())){
           movieTitleAsked = listOfMovies.get(i).getTitle();
  ...
  public static void movieChatFunction(String userInput, String movieTitleAsked){
    ...
    for(int i=0; i<movieQuestion.size();i++){
      // if the user input contains one of the possible movie questions key word, we respond appropriately
        if(userInput.contains(movieQuestion.get(i))){
